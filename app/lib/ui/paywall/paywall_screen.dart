...
class _PaywallScreenState extends ConsumerState<PaywallScreen> {
  ...
  Future<void> _initializeIAP() async {
    ...
    try {
      ...
      setState(() { _error = 'Failed to load subscription details.'; });
    } finally {
  ...
  Future<void> _applyDiscount() async {
    ...
    try {
      final service = ref.read(subscriptionServiceProvider);
      final discount = await service.getDiscount(_discountController.text.trim());
      setState(() { _discount = discount; });
      
      if (discount <= 0) {
        setState(() { _error = 'Invalid discount code.'; });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Discount of \$${discount.toStringAsFixed(2)} applied!'))
        );
      }
    } catch (e) {
      setState(() { _error = 'Failed to apply discount code.'; });
    } finally {
      setState(() { _loading = false; });
    }
  }
  ...
  Future<void> _subscribe() async {
    ...
    try {
      ...
      setState(() { _error = 'Subscription failed: ${e.toString()}'; });
    } finally {
      setState(() { _loading = false; });
    }
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Center(
                      child: Column(
                        children: [
                          const Icon(Icons.cloud_done, size: 64, color: Colors.blue),
                    ...
                    const SizedBox(height: 16),
                    _buildFeatureItem('Unlimited cloud storage', Icons.cloud_upload),
                    _buildFeatureItem('Ad-free experience', Icons.block),
                    _buildFeatureItem('Priority customer support', Icons.support_agent),
                    _buildFeatureItem('Exclusive filters and effects', Icons.auto_fix_high),
                    _buildFeatureItem('High-resolution backup', Icons.high_quality),
                    
                    const SizedBox(height: 32),
                    
                    // Discount code
                    const Text('Have a discount code?', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
                    const SizedBox(height: 8),
                    TextField(
